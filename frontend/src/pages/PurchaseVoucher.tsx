...
export default function PurchaseVoucher() {
  ...
  const fetchData = async () => {
    ...
    const [supRes, stockRes, vouchRes] = await Promise.all([
      fetch(
        `http://localhost:5000/api/ledgers/${activeCompany.id}?type=supplier`,
      ),
      fetch(`http://localhost:5000/api/stock-items/${activeCompany.id}`),
      fetch(
        `http://localhost:5000/api/vouchers/${activeCompany.id}?type=purchase`,
      ),
    ]);

    const [supData, stockData, vouchData] = await Promise.all([
      supRes.json(),
      stockRes.json(),
      vouchRes.json(),
    ]);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5000/api/vouchers/purchase", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          company_id: activeCompany?.id,
          ledger_id: selectedSupplier,
      ...
      if (!res.ok) {
        ...
        setSuccess(`Purchase Voucher ${data.voucher_no} created successfully!`);
        setSelectedSupplier("");
        setItems([]);
        setShowForm(false);
        fetchData();
        ...
        {showForm && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              <div>
                <label htmlFor="supplier" className={labelCls}>
              ...
              <div>
                <label htmlFor="v_date" className={labelCls}>
                  Voucher Date *
                </label>
                <input
                  id="v_date"
            ...
            <div className="bg-slate-50 rounded-xl p-4 mb-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                <div className="col-span-2">
                  <label htmlFor="stock_item" className={labelCls}>
                    Item Name *
                  </label>
                  <select
                    id="stock_item"
                    ...
                    onChange={(e) => {
                      ...
                      if (stock) setRate(String(stock.purchase_price));
                    }}
                    className={inputCls}
                  >
                ...
                <div>
                  <label htmlFor="p_qty" className={labelCls}>
                    Qty {selectedStock ? `(${selectedStock.unit})` : ""}
                  </label>
                  <input
                    id="p_qty"
                    type="number"
                ...
                <div>
                  <label htmlFor="p_rate" className={labelCls}>
                    Rate (₹/unit)
                  </label>
                  <input
                    id="p_rate"
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? "Saving..." : "Save Purchase Voucher"}
              </button>
              <button
        ...
        {vouchers.length === 0 ? (
          ...
          <div className="overflow-x-auto bg-white rounded-xl border border-slate-200 shadow-sm">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {["Voucher No.", "Date", "Supplier", "Items", "Total"].map(
                    (h) => (
                      <th
                        key={h}
